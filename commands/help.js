...
module.exports.run = async (client, message, args) => {
  ...
  client.cmdhelp
    ...
    .forEach(cmd => {
      generalArr.push(cmd.name);
    });
  client.cmdhelp
    .filter(cmd => cmd.category === "Debug")
    .forEach(cmd => {
      debugArr.push(cmd.name);
    });
  ...
  if (!args[0]) {
    ...
    let cmdEmbed = new Discord.RichEmbed().setColor(0xff4500);
